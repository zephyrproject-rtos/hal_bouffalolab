...
/*
 * Minimal modern-BSD red-black tree interface. The wl80211 blob carries the
 * full RB_GENERATE() implementation (FreeBSD sys/tree.h, rbe_link[3] node
 * layout); the host side only needs the node/head macros, forward declarations
 * of the exported tree functions, and the forward iterator. Claiming the
 * _SYS_TREE_H_ guard keeps picolibc's ABI-incompatible <sys/tree.h> out.
 */
#ifndef _SYS_TREE_H_
#define _SYS_TREE_H_
...
#define RB_PROTOTYPE(name, type, field, cmp)				\
	...
	struct type *name##_RB_REINSERT(struct name *, struct type *)
#define RB_MIN(name, head)	name##_RB_MINMAX(head, RB_NEGINF)
#define RB_NEXT(name, x, elm)	name##_RB_NEXT(elm)
...
struct wl80211_scan_ops {
	...
	void (*scan_ap_ind)(struct wl80211_scan_ops *, struct wl80211_scan_ap_ind *, uint8_t *,
			    uint16_t);
...
struct wl80211_connect_params {
	 ...
	 * In frames sent from stations to access points (association request, reassociation
	 * request), the Listen Interval field is used to indicate to the AP how often a station in
	 * Power Save mode wakes to listen to beacon management frames. This value is an integer
	 * expressed in beacon interval units (for example, a value of 3 indicates that the station
	 * wakes up every three beacons). This field is 2 bytes long, which means that the maximum
	 * interval could be 65,535.
	 */
...
typedef void (*wl80211_monitor_rx_cb_t)(void *ctx, void *pkt, size_t len, size_t mac_hdr_len,
					int rssi);
...
typedef int (*wl80211_input_cb_t)(void *prv, uint8_t vif_type, void *rxhdr, void *buf,
...
static inline int wl80211_freq_check(int freq)
{
	...
	for (i = 0; i < wl80211_glb.country->channel24G_num; i++) {
		if (freq ==
		    _channel_to_freq(WL80211_BAND_2G4, wl80211_glb.country->channel24G_chan[i])) {
	...
	for (i = 0; i < wl80211_glb.country->channel5G_num; i++) {
		if (freq ==
		    _channel_to_freq(WL80211_BAND_5G, wl80211_glb.country->channel5G_chan[i])) {
...
{
	return wl80211_cntrl(WL80211_CTRL_AP_DEAUTH_STA, (unsigned int)sta_idx,
			     (unsigned int)reason);
}
